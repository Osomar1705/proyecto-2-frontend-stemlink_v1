...
export function Button({ variant = 'primary', loading, children, className = '', disabled, type = 'button', ...props }: Props) {
  return (
    <button
      ...
      className={`surface-interactive group inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 [&>svg]:transition-transform [&>svg]:duration-200 [&:hover>svg]:translate-x-0.5 ${variants[variant]} ${className}`}
    >
