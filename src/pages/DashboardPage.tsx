    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="flex items-center gap-4">
          <div className="p-3 bg-indigo-100 rounded-xl"><Calendar className="text-indigo-600" size={24} /></div>
          <div>
            <p className="text-2xl font-bold text-gray-900">{confirmed.length}</p>
            <p className="text-sm text-gray-500">Sesiones confirmadas</p>
          </div>
        </Card>
        <Card className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          {confirmed.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6">No tienes sesiones próximas</p>
          ) : (
            <div className="space-y-3">
              {confirmed.slice(0, 3).map(s => (
                <div key={s.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-gray-800">{s.topic}</p>
